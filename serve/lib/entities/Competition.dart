/// Competition.dart
/// Version 0.0.1a
/// Copyright 2016 Programmers for a Better Tomorrow, Yesterday

part of entities;

/// Competition
/// This is an autogenerated class mapping the MySQL Table Competition to an AvacadORM Entity.

/// Subclass of Organization_Role that represents Competitors to the business.
@Table('Competition')
class Competition extends Entity {
	// ***Table Columns***
	@Column.PrimaryKey('id')
	int id;
	@Column('Organization_Role_id')
	int Organization_Role_id;

	// ***Table Foreign Key Entities***
	@Column.ManyToOneForeignKey('Organization_Role_id')
	Role_of_Organization role_of_organization;
}